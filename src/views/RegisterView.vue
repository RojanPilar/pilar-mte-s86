<template>
  <div class="min-h-screen bg-cinema-black flex items-center justify-center px-4 relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none">
      ...
      <div class="absolute bottom-0 right-0 w-64 h-64 bg-cinema-gold/5 rounded-full blur-3xl"></div>
    </div>

    <div class="w-full max-w-md relative z-10 animate-slide-up">
      <!-- Logo -->
      <div class="text-center mb-8">
        <RouterLink to="/login" class="inline-flex items-center gap-2 group mb-6">
          <div class="w-10 h-10 bg-cinema-red rounded-xl flex items-center justify-center shadow-red-glow group-hover:scale-110 transition-transform">
        ...
        <h1 class="text-3xl font-bold text-cinema-text mb-2">Create account</h1>
        <p class="text-cinema-subtle">Join to start exploring movies</p>
      </div>

      <!-- Card -->
      <div class="card glow-red p-8">
        <AlertMessage :message="errorMsg" type="error" class="mb-5" />
        <AlertMessage :message="successMsg" type="success" class="mb-5" />
        ...
        <form @submit.prevent="handleRegister" novalidate class="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              required
            />
            <p v-if="passwordMismatch" class="error-text mt-1">Passwords do not match.</p>
          </div>
        ...
        <p class="text-center text-cinema-subtle text-sm mt-6">
          Already have an account?
          <RouterLink to="/login" class="text-cinema-red hover:text-red-400 font-medium transition-colors">Sign in</RouterLink>
        </p>
      </div>
    </div>
  </div>
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    await registerUser({ email: form.value.email, password: form.value.password })
